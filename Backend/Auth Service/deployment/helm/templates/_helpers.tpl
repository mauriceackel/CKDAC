...
*/}}
{{- define "auth-service.internalGatewayUrl" -}}
...
{{- define "auth-service.gatewayIds" -}}
...
{{- range $idx, $val := (kindIs "string" .gatewayName | ternary (list .gatewayName) .gatewayName) -}}
{{- if ne $idx 0 }}|{{end -}}
{{ include "auth-service.escapeRegex" (get $.apiGateway $val).hostname }}
{{- end }}
{{- end }}

{{- define "auth-service.escapeRegex" -}}
{{ regexReplaceAll "([.*?])" . "\\\\${1}"}}
{{- end }}